<div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-4 mb-4 flex gap-3 items-center">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="بحث بالاسم أو التخصص..." class="border border-slate-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-orange-400 flex-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
            <div class="flex items-start justify-between mb-3">
                ...
                <span class="text-xs {{ $center->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }} rounded-full px-2 py-0.5">{{ $center->is_active ? 'نشط' : 'موقوف' }}</span>
            </div>
            @if($center->specialization)<p class="text-xs bg-purple-100 text-purple-700 inline-block rounded-full px-2 py-0.5 mb-2">{{ $center->specialization }}</p>@endif
        ...
        @empty
        <div class="col-span-3 bg-white rounded-xl p-16 text-center text-slate-400">لا توجد مراكز</div>
        @endforelse
    </div>
    ...
    <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                <h3 class="font-bold text-slate-800">{{ $editingId ? 'تعديل المركز' : 'إضافة مركز صيانة' }}</h3>
            ...
            <div class="p-6 space-y-4">
                ...
                <div><label class="block text-slate-700 text-sm font-medium mb-1">التخصص</label><input wire:model="specialization" type="text" placeholder="مثال: كهرباء سيارات، ميكانيكا..." class="w-full border border-slate-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400"></div>
                @if(!$editingId)
                ...
                @endif
                @if($editingId)<div class="flex items-center gap-2"><input wire:model="is_active" type="checkbox" id="is_active" class="rounded"><label for="is_active" class="text-sm text-slate-700">المركز نشط</label></div>@endif
            </div>
